...
- `README.md`：新增「Visual Mode」说明，决策流程图并入模式判定，版本号对齐；新增「Installation（安装方法）」「Usage（使用方法）」章节，安装方法改为「网页下载 ZIP + 复制文件夹」三步傻瓜版（零命令行）；安装对照表加入 Qoder，并补充 Figma / 墨刀设计工具的使用说明，便于团队快速安装与上手。
