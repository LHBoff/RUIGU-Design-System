...
- 筛选项 ≤ 3 → 同一行；> 3 → 自动换行，每行最多 3 个；
- 查询（primary）、重置（default）按钮紧跟最后一个筛选项之后；
- 用 Ant Design `Form`（`layout="inline"` 或 grid）组织，不用自定义 div 拼排。
- **Visual Mode（模式前提，见 SKILL.md 第 2 节）：** 默认组件规范优先模式下，即使原型图把筛选项画成上下单列，也一律改为上述水平多列规范；仅视觉参考模式（设计稿/高保真）保留图片布局。
...
**Visual Mode（模式前提，见 SKILL.md 第 2 节）：** 设计图分析用于识别页面目的、区域、页面模式与组件，属结构 / 功能理解。默认组件规范优先模式下，图片视觉（颜色、字号、圆角、阴影、间距等）不参与最终实现，页面用 Ant Design 默认规范生成；仅当指令含"设计稿/设计图/高保真"等指向性文案（视觉参考模式）时，才参考图片视觉定制。
