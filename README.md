...
RUIGU 默认按组件库规范生成页面，避免被上传图片的视觉干扰：

* 默认（组件规范优先）：上传原型图 / 设计图 / 截图时，图片仅作结构、内容、功能、交互参考，视觉一律采用 Ant Design 默认样式（默认蓝色、8px 网格、默认排版），不参考图片的颜色 / 圆角 / 阴影 / 字体。
* 仅当指令包含指向性文案（如"设计稿 / 设计图 / 高保真 / 视觉还原"）时，才进入视觉参考模式，在 Ant Design 组件基础上按图片视觉定制。

如需参考图片视觉，在需求中说明即可，例如"照着这张**设计稿**的样式做"；如需切回默认，说明"用**默认组件样式**"。
