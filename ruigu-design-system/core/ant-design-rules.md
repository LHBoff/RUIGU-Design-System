...
- 无论输入是截图、UI 设计图、原型还是 Figma 设计，**默认组件规范优先**：图片只提供内容 / 功能 / 交互（结构布局不采用，按 Ant Design 页面模式规范重新组织），视觉（颜色、字体、圆角、阴影、间距、图标风格等）一律忽略，页面用 Ant Design 默认规范实现。
...
Input:
Prototype / Screenshot（仅作内容与功能参考，结构按规范重新组织）
